...
string myPdfNameSig = "Hypatia_BPHTreshold";
string myPdfNameBkg = "DoubleSidedCB_D0BG";
...
bool CreateModelPdf(RooWorkspace& Ws, string pdfName, bool bConstrainedFit = false)
{
	...
	if (pdfName.compare("DoubleSidedCB_BPHTreshold") == 0)
	{
		...
		Ws.factory("SUM::signal(chic1, c2ratio[0.3,0.00,1.0]*chic2)");
	...
	else if (pdfName.compare("DoubleSidedCB_D0BG") == 0)
	{
		...
		Ws.factory("SUM::DoubleSidedCB_D0BG(nsig[5000,2,20000]*signal, nbkg[1800,10,50000]*background)");
		return true;
	}
	else if (pdfName.compare("Hypatia_BPHTreshold") == 0)
	{
		...
		Ws.factory("SUM::signal(chic1, c2ratio[0.3,0.0,1.0]*chic2)");

		Ws.factory("expr::delta('rvmass-qzero', rvmass, qzero[3.2])");
		Ws.factory("expr::b1('beta_bkg*(rvmass-qzero)', rvmass, qzero, beta_bkg[-2, -3.3, -0.5])");
		//Ws.factory("EXPR::background('pow(delta,alpha_bkg)*exp(b1)',delta, b1, alpha_bkg[0.3, 0.1, 0.8])");
		Ws.factory("EXPR::background('pow(delta,alpha_bkg)*exp(b1)',delta, b1, alpha_bkg[0.2, 0.1, 0.8])"); //normal settings
		...
		Ws.factory("SUM::Hypatia_BPHTreshold(nsig[50,0,20000]*signal, nbkg[2000,0,10000]*background)"); //normal settings
		//Ws.factory("SUM::Hypatia_BPHTreshold(nsig[300,0,2000]*signal, nbkg[4000,0,10000]*background)");
		return true;
	}
...
int DetermineSystematicsBin(double* bins, int binN, bool isSingleBin=false, string fittingSetName = "", int nBinSet=0, const char* fileInNominal = "", const char* fileConstraintsNomSig = "", const char* fileConstraintsAltSig = "") { //uses global variables

	RooAbsReal::defaultIntegratorConfig()->setEpsAbs(5e-8); //shouldn't really be here, trying to prevent slow-down
	RooAbsReal::defaultIntegratorConfig()->setEpsRel(5e-8);


	//create the directory:
	string saveDirName = (string)"Output_systChic/"+fittingSetName + Form("/bin_%i", binN);
	...
	for (int iToy = 0; iToy < 100; iToy++)
	{
		...
		RooArgSet*  cols = new RooArgSet(*rvmass, *rvpt, *rvrap, *rvntrack);
		...
		CreateModelPdf(myWs, myPdfName, true);
		CreateModelPdf(myWsNom, myPdfName, true);
		CreateModelPdf(myWsSig, myPdfNameSig, true);
		CreateModelPdf(myWsBkg, myPdfNameBkg, true);


		// setConstraints

		int flagConstrSet = SetConstraints(myWs, bins, binN, fittingSetName, myPdfName, fileConstraintsNomSig);
		if (flagConstrSet == -1) cout << "Setting constrains failed " << endl;
		...
		int flagConstrSetNom = SetConstraints(myWsNom, bins, binN, fittingSetName, myPdfName, fileConstraintsNomSig);
		if (flagConstrSetNom == -1) cout << "Setting constrains failed " << endl;

		int flagConstrSetAlt = SetConstraints(myWsSig, bins, binN, fittingSetName, myPdfNameSig, fileConstraintsAltSig);
	//	int flagConstrSetAlt = SetConstraints(myWsSig, bins, 3, fittingSetName, myPdfNameSig, fileConstraintsAltSig);  //test
		if (flagConstrSetAlt == -1) cout << "Setting constrains alternate signal failed " << endl;

		int flagConstrSetBkg = SetConstraints(myWsBkg, bins, binN, fittingSetName, myPdfNameBkg, fileConstraintsNomSig);
		if (flagConstrSetBkg == -1) cout << "Setting constrains alternate background failed " << endl;
		...
		RefreshModel(myWsNom, myPdfName, false);
		RefreshModel(myWsSig, myPdfNameSig, false);
		RefreshModel(myWsBkg, myPdfNameBkg, false);
		...
		RooDataSet* rdsToy = myWs.pdf(myPdfName.c_str())->generate(*rvmass, Extended(kTRUE));
...
//		gDirectory->ls();
		...
		myWs.pdf(myPdfName.c_str())->plotOn(massframeBin, Components("background"), LineStyle(kDashed), LineColor(kGray));
		myWs.pdf(myPdfName.c_str())->plotOn(massframeBin, Components("chic1"), LineStyle(kDashed), LineColor(kGray));
		myWs.pdf(myPdfName.c_str())->plotOn(massframeBin, Components("chic2"), LineStyle(kDashed), LineColor(kGray));
		//
		//myWsNom.Print();
		//myWsSig.Print();
		//myWsNom.pdf(myPdfName.c_str())->fitTo(*myWs.data("rdsToy1"));
		RooFitResult* fitResultNom = myWsNom.pdf(myPdfName.c_str())->fitTo(*rdsToy, Extended(true), SumW2Error(true), PrintLevel(-1), Range(mass_windowFit_l, mass_windowFit_h), NumCPU(8), Save(true));
		myWsNom.pdf(myPdfName.c_str())->plotOn(massframeBin, Name("FullPdfNom"), LineColor(kBlue));
		//myWsNom.pdf(myPdfName.c_str())->plotOn(massframeBin, Name(TString::Format("FullPdfNom%i", iToy)), LineColor(kBlue));
		//myWsNom.pdf(myPdfName.c_str())->paramOn(massframeBin, Layout(0.55));
		myWsNom.pdf(myPdfName.c_str())->plotOn(massframeBin, Components("background"), LineStyle(kDashed), LineColor(kBlue));
		myWsNom.pdf(myPdfName.c_str())->plotOn(massframeBin, Components("chic1"), LineStyle(kDashed), LineColor(kBlue));
		myWsNom.pdf(myPdfName.c_str())->plotOn(massframeBin, Components("chic2"), LineStyle(kDashed), LineColor(kBlue));

		//cout << "NomFitStatus: " << fitResultNom->status() << endl;

		double sigNomCount = myWsNom.var("nsig")->getValV();
		...
		RooFitResult* fitResultSig = myWsSig.pdf(myPdfNameSig.c_str())->fitTo(*rdsToy, Extended(true), SumW2Error(true), PrintLevel(-1), Range(mass_windowFit_l, mass_windowFit_h), NumCPU(8), Save(true));
		...
		//myWsSig.pdf(myPdfNameSig.c_str())->paramOn(massframeBin, Layout(0.55));
		myWsSig.pdf(myPdfNameSig.c_str())->plotOn(massframeBin, Components("background"), LineStyle(kDashed), LineColor(kRed));
		myWsSig.pdf(myPdfNameSig.c_str())->plotOn(massframeBin, Components("chic1"), LineStyle(kDashed), LineColor(kRed));
		myWsSig.pdf(myPdfNameSig.c_str())->plotOn(massframeBin, Components("chic2"), LineStyle(kDashed), LineColor(kRed));

		////cout << "SigFitStatus: " << fitResultSig->status() << endl;
		double sigAltCount = myWsSig.var("nsig")->getValV();
		...
		//if (fitResultNom->status() != 0 || fitResultSig->status() != 0)
		//{
		//	nFailedSig++;
		...
		RooFitResult* fitResultBkg = myWsBkg.pdf(myPdfNameBkg.c_str())->fitTo(*rdsToy, Extended(true), SumW2Error(true), PrintLevel(-1), Range(mass_windowFit_l, mass_windowFit_h), NumCPU(8), Save(true));
		myWsBkg.pdf(myPdfNameBkg.c_str())->plotOn(massframeBin, Name("FullPdfBkg"), LineColor(kGreen+2));
		//myWsBkg.pdf(myPdfNameBkg.c_str())->paramOn(massframeBin, Layout(0.55));
		myWsBkg.pdf(myPdfNameBkg.c_str())->plotOn(massframeBin, Components("background"), LineStyle(kDashed), LineColor(kGreen+2));
		myWsBkg.pdf(myPdfNameBkg.c_str())->plotOn(massframeBin, Components("chic1"), LineStyle(kDashed), LineColor(kGreen+2));
		myWsBkg.pdf(myPdfNameBkg.c_str())->plotOn(massframeBin, Components("chic2"), LineStyle(kDashed), LineColor(kGreen+2));


		double bkgAltCount = myWsBkg.var("nsig")->getValV();
		...
		//if (fitResultNom->status() != 0 || fitResultBkg->status() != 0)
		//{
		//	nFailedBkg++;
		//	cout << "A FIT HAS FAILED, RESULT SKIPPED" << endl;
		//}
		...
		TPaveText *textchiCounts = new TPaveText(.76, .72, .94, .92, "brNDC");
...
//		chi2
		...
		//TObject* testObj = FindObject("DoubleSidedCB_BPHTreshold_Int[rvmass|fit_nll_DoubleSidedCB_BPHTreshold_rdsToy0]_Norm[rvmass]");
		...
		cFit->SaveAs(((string) saveDirName+"/ChicToys-" + fittingSetName +"_bin_"+ Form("%i_toy_%i.png",binN, iToy)).c_str());
		cFit->SaveAs(((string)saveDirName + "/ChicToys-" + fittingSetName + "_bin_" + Form("%i_toy_%i.pdf", binN, iToy)).c_str());
		...
		if (fitResultNom->status() != 0 || fitResultSig->status() != 0 || chi2_fitSig > 5.0 || chi2_fitNom > 5.0) // store only non-failed fits. We check the output, if there is only a few failed, skipping them should be okay
		{
			nFailedSig++;
			cout << "A FIT HAS FAILED, RESULT SKIPPED" << endl;
		}
		...
		//cout << "Printing all vars" << endl;
		//myWsNom.allVars().Print();

		//myWsNom.allPdfs().Print();

		//cout << endl<< "Printing all args for pdf: " << endl;
		//myWsNom.pdf(myPdfName.c_str())->printArgs(std::cout);

		//myWsNom.RecursiveRemove(myWsNom.pdf(myPdfName.c_str()));
		...
		//myWsSig.Print();
		//rdsToy->Print("V");
		//cout << endl << endl << "Massframe bin" << endl;
		//massframeBin->Print("V");
		//massframeBin->remove(TString::Format("FullPdfNom%i", iToy));
		//massframeBin->Print("V");
		//cout << endl << endl<< "gDir:" << endl;
		////massframeBin->remove(TString::Format("FullPdfNom%i", iToy));
		...
		////massframeBin->Print("V");
		////cout << endl << endl;
		//cout << endl << endl << "pad1:" << endl;

		//pad1->ls();
		//cout << endl << endl << "cFit:" << endl;
	
		//cFit->ls();


		//massframeBin->getCurve("FullPdfNom")->Dump();
		//massframeBin->getCurve("FullPdfNom")->Print("V");
	...
	cDiff->SaveAs(((string)saveDirName + "/ChicToys" + fittingSetName + Form("Result_bin%i", binN) + ".png").c_str());
	cDiff->SaveAs(((string)saveDirName + "/ChicToys" + fittingSetName + Form("Result_bin%i", binN) + ".root").c_str());
	cDiff->SaveAs(((string)saveDirName + "/ChicToys" + fittingSetName + Form("Result_bin%i", binN) + ".pdf").c_str());
	...
	cDiff2->SaveAs(((string)saveDirName + "/ChicToys" + fittingSetName + Form("ResultZoomedOut_bin%i", binN) + ".png").c_str());
	cDiff2->SaveAs(((string)saveDirName + "/ChicToys" + fittingSetName + Form("ResultZoomedOut_bin%i", binN) + ".root").c_str());
	cDiff2->SaveAs(((string)saveDirName + "/ChicToys" + fittingSetName + Form("ResultZoomedOut_bin%i", binN) + ".pdf").c_str());
...
int DetermineSystematicsSet(double* bins, int nbins, /*RooRealVar* rvmass, RooArgSet*  cols, RooWorkspace& myWs, RooWorkspace& myWsNom, RooWorkspace& myWsSig, RooWorkspace& myWsBkg,*/ string fittingSetName = "", int BinToFit=-1, bool bConstrainedFit = false, const char* fileInNominal = "", const char* fileConstraintsNomSig = "", const char* fileConstraintsAltSig = "")
{
	string saveDirName = (string)"Output_systChic/" + fittingSetName;
	cout << saveDirName << endl;
	string cmd = string("mkdir ") + string(saveDirName);
	system(cmd.c_str());
	...
	if (BinToFit == -1) { // do all
		...
		{
			DetermineSystematicsBin(bins, binN, false, fittingSetName, nBinSet, fileInNominal, fileConstraintsNomSig, fileConstraintsAltSig);
		}
	}
	else
	{
		if (BinToFit < nbins) { DetermineSystematicsBin(bins, BinToFit, true, fittingSetName, nBinSet, fileInNominal, fileConstraintsNomSig, fileConstraintsAltSig); }
...
//void SystFits_Chic(int SetToFit = 0, int BinToFit = -1, const char* fileInNominal = "Chi_c_output_Nominal_v2-bothDir_DCB.root", const char* fileOut = "Chi_c_Syst.root", bool flagConstrainedFit = true, const char* fileConstraintsNomSig = "Chi_c_constraints_Officialv3_NarrowRangeDCBW.root", const char* fileConstraintsAltSig = "Chi_c_constraints_Officialv3_NarrowRangeHypatiaW.root")
void SystFits_Chic(int SetToFit = 0, int BinToFit = -1, const char* fileInNominal = "Chi_c_output_Nominal_vDissertation-bothDir_DCB_NewBins.root", const char* fileOut = "Chi_c_Syst_vDissertation.root", bool flagConstrainedFit = true, const char* fileConstraintsNomSig = "Chi_c_constraints_OfficialvDissertation_NarrowRangeDCBW_NewBins.root", const char* fileConstraintsAltSig = "Chi_c_constraints_OfficialvDissertation_NarrowRangeHypatiaW_NewBins.root")
{
	...
	string cmd = string("mkdir Output_systChic/");
	system(cmd.c_str());
	...
	if (SetToFit==1){ DetermineSystematicsSet(bins_y, nbins_y, myFittingSetName, BinToFit, flagConstrainedFit, fileInNominal, fileConstraintsNomSig, fileConstraintsAltSig); }
	//else if (SetToFit==2 || SetToFit==12){ DetermineSystematicsSet(bins_nTrk, nbins_nTrk, myFittingSetName, BinToFit, flagConstrainedFit, fileInNominal, fileConstraintsNomSig, fileConstraintsAltSig); }
	else if (SetToFit == 5) { DetermineSystematicsSet(bins_nTrk, nbins_nTrk, myFittingSetName, BinToFit, flagConstrainedFit, fileInNominal, fileConstraintsNomSig, fileConstraintsAltSig); }
	else { DetermineSystematicsSet(bins_pT, nbins_pT, myFittingSetName, BinToFit, flagConstrainedFit, fileInNominal, fileConstraintsNomSig, fileConstraintsAltSig); }
