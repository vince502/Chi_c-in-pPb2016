...
const TString sNameTag = "_pT_models";
...
int plotNominal_pT_models()
{
	...
	// Load Vinc's ICEM calculation

	TGraph* gICEM_base = new TGraph("Vincent_ratio-base.txt", "%lg %lg", "");
	TGraph* gICEM_high = new TGraph("Vincent_ratio-high.txt", "%lg %lg", "");
	TGraph* gICEM_low = new TGraph("Vincent_ratio-low.txt", "%lg %lg", "");
	...
	TGraphAsymmErrors* gAS_ICEM = new TGraphAsymmErrors(gICEM_base->GetN(), gICEM_base->GetX(), gICEM_base->GetY()); // Make AsymmErrors

	for (int i = 0; i < gICEM_base->GetN(); i++) //fill the error bars
	{
		gAS_ICEM->SetPointEYhigh(i, (gICEM_high->GetPointY(i) - gICEM_base->GetPointY(i)));
		gAS_ICEM->SetPointEYlow(i, (gICEM_base->GetPointY(i) - gICEM_low->GetPointY(i)));
	}

	//plotting

	gICEM_base->SetLineColor(4);
	gICEM_base->SetLineWidth(2);
	gICEM_low->SetLineColor(4);
	gICEM_high->SetLineColor(4);
	//gICEM_high->Draw("ZPE");
	gAS_ICEM->SetLineColor(4);
	gAS_ICEM->SetLineWidth(2);
	gAS_ICEM->SetFillColor(4);
	gAS_ICEM->SetFillStyle(3015);
	gAS_ICEM->Draw("L3");
	//gICEM_low->Draw("L");
	//gICEM_high->Draw("L");
	//gICEM_base->Draw("L");
	...
	TPaveText* pText1 = new TPaveText(0.48, 0.8, 0.78, 0.88, "NDC NB");
	pText1->AddText("-2.9 < y_{CM} < 1.9");
	pText1->SetTextSize(0.048);
	pText1->SetFillColor(0);
	pText1->Draw("");

	//TLegend*leg2 = new TLegend(0.21, 0.21, 0.51, 0.28, "");
	//leg2->SetFillColor(kWhite);
	//leg2->SetBorderSize(0);
	//leg2->SetTextFont(42);
	//leg2->SetTextSize(0.044);
	//leg2->AddEntry(gAS_ICEM, "#splitline{ICEM, pPb}{-2.9 < y_{CM} < 1.9}", "f");
	////leg->AddEntry(gAS_Result3, "1.0<|y|<2.4", "p");
	//leg2->Draw("same");
